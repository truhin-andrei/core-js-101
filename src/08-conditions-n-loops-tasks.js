...
function isInsideCircle(circle, point) {
  const distToPoint = Math.sqrt((circle.center.x - point.x) ** 2
    + (circle.center.y - point.y) ** 2);
  if (distToPoint >= circle.radius) {
    return false;
  }
  return true;
}
...
function findFirstSingleChar(str) {
  const cash = [];
  for (let chN = 0; chN < str.length; chN += 1) {
    let i = chN;
    while (i < str.length - 1) {
      ...
      if (str[chN] === str[i]) {
        ...
        cash.push(str[chN]);
        break;
      } else if ((i === str.length - 1) && !cash.includes(str[chN])) {
        return str[chN];
      }
    }
  }
  ...
  for (let i = 0; i < len; i += 1) {
    for (let k = 0; k < arr.length - 1; k += 1) {
      if (arr[k][i] !== arr[k + 1][i]) {
        k = arr.length;
        break;
      } else if (arr[k][i] === arr[k + 1][i] && k === arr.length - 2) {
        if (arr[k][i] === ' ') {
          result += '/';
        }
        result += `${arr[k][i]}/`;
